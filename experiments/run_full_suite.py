...
def update_params(dataset, seed):
    ...
    if dataset == "asia":
        ...
        params["core_logic"]["consensus_momentum"] = 0.60
        params["simulation"]["target_label"] = 'auto'
        params["simulation"]["poison_feature"] = 0
    elif dataset == "alarm":
...
def main():
    ...
    for dataset in DATASETS:
        ...
        for seed in SEEDS:
            print(f"\n=============================================")
            print(f"RUNNING: Dataset={dataset.upper()}, Seed={seed}")
            print(f"=============================================")
    ...
    # Aggregate and Save
    summary_path = PROJECT / "results" / "ASR_MTA_Table.md"
    summary_path.parent.mkdir(exist_ok=True)
    with open(summary_path, "w") as f:
        f.write("# Attack Success Rate (ASR) and Main Task Accuracy (MTA)\n\n")
    ...
    print(f"\nFinal results aggregated to {summary_path}")
