...
since poisoned models can preserve statistical similarity to honest updates
while corrupting their underlying reasoning structure.
...
topological inspection of latent feature dependencies. In \por{}, each client
applies the \notears{} algorithm to extract a Directed Acyclic Graph~(DAG)
encoding its reasoning. A server-side \simgnn{} Logic Validator compares
each DAG against a global consensus, approximating Graph Edit Distance~(GED)
in $\mathcal{O}(1)$ time. An auxiliary Causal Effect Distance~(CED) audits
coefficient magnitudes to catch weight-only backdoors. Honest clients produce
DAGs topologically close to the consensus; poisoned clients exhibit detectable
structural deformation.

\por{} is demonstrated within \fedneat{}, a heterogeneous neuroevolutionary
framework, and validated in an Algorithmic Finance environment against a
3-adversary threat model (Temporal Mimicry, Topology Inversion, Gradient
Mimicry). Extensive evaluation on Bayesian networks shows that \por{} reduces
Attack Success Rate from $\mathbf{60.30\%}$ to $\mathbf{0.40\%}$ on ALARM and from
$\mathbf{5.62\%}$ to $\mathbf{0.52\%}$ on ASIA, with zero false positives.
\end{abstract}

\begin{IEEEkeywords}
Federated learning, backdoor defense, explanation poisoning, causal discovery,
graph neural networks, Byzantine robustness, structural auditing, NOTEARS,
...
collection~\cite{kairouz2021advances,li2020fedsurvey}. Recent advances
extend FL toward \textit{agentic} deployments, where participants are
autonomous agents that independently curate data, optimise local objectives,
and interact with dynamic environments. While this increases flexibility and
personalisation, it also introduces opacity: an agentic node can independently
alter \textit{how} it learns, enabling a new class of attacks without
triggering statistical alarms.
...
deformation of the causal DAG without destroying the backdoor itself---a
mathematical contradiction we exploit as the core defense principle.
...
challenging substrate in which to evaluate \por{}. We further validate \por{}
in an Algorithmic Finance environment---an agentic setting with a Transformer
Actor-Critic policy, continuous observation spaces, curriculum learning, and
three simultaneous adversary types---demonstrating domain-agnostic
generalisability. The system is implemented using the Flower federated
...
\begin{enumerate}
  \item \textbf{The \por{} Framework}: A novel structural auditing mechanism
        ...
        clients, operating orthogonally to weight-space defenses and
        defeating Explanation Poisoning by exploiting the mathematical
        contradiction between causal-graph stealth and backdoor effectiveness.

  \item \textbf{Dual-Gate Logic Validation (GED + CED)}: A two-stage
        server-side validator combining \simgnn{}-based GED approximation
        in $\mathcal{O}(1)$ inference time with a MAD-calibrated Causal
        Effect Distance gate, together catching both topology-altering and
        weight-only backdoor variants.
  ...
  \item \textbf{Finance Domain Validation}: Empirical evaluation on a
        Transformer Actor-Critic agentic FL environment with three
        simultaneous adversary types (Temporal Mimicry, Topology Inversion,
        Gradient Mimicry), demonstrating PoR's generalisability beyond
        Bayesian network benchmarks to complex sequential decision processes.
  ...
  \item \textbf{Formal Privacy Guarantee}: Differential Privacy via DP-SGD
        (RDP composition) with $(\varepsilon\approx 2.0,\,\delta=10^{-5})$-DP
        after 5 federation rounds in the Finance environment, showing that
        PoR's adversary filtering additionally reduces the effective privacy
        budget by excluding adversarial gradients from aggregation.
\end{enumerate}
...
Poisoning attacks, which are deliberately clean and structured, allowing
them to pass reconstruction-based filters. ESFL~\cite{esfl2025} (IEEE
TDC 2025) employs adaptive local differential privacy and compressive
sensing for security, but the injected noise paradoxically aids attackers
by obscuring malicious triggers. None of these approaches inspect the
...
Attacks in federated network intrusion detection, where an adversary
embeds a persistent bias while simultaneously manipulating the model to
conceal this bias from SHAP. Slack et al.~\cite{slack2020fooling}
further showed that LIME and SHAP explanations can be systematically
falsified by training an Out-of-Distribution~(OOD) detector that switches
model behaviour between inference and explanation modes.
Heo et al.~\cite{heo2019fooling} showed that gradient-based saliency maps
are similarly manipulable. These attacks formalise as the dual-objective
optimisation of Equation~\eqref{eq:exppoison} and are particularly
dangerous in FL, as a trusted model can present clean explanations to a
central auditor while remaining malicious in deployment.
\por{} defends against Explanation Poisoning by auditing latent structure
rather than output-space explanations, rendering the OOD-switching mechanism
...
with MSE of $1.19 \times 10^{-3}$. GRAPHEDX~\cite{graphedx2024} (NeurIPS
2024) introduced Neural Set Divergence as a GNN-based approximation of GED
estimable in milliseconds, confirming the feasibility of real-time
graph-based governance at federation scale. The Message Passing Neural
...
\subsection{Federated Causal Discovery}

Federated Causal Discovery~(FCD) has emerged as a research direction
aiming to learn causal graphs from distributed, privacy-preserving data.
FedECD~\cite{fedecd2024} (NeurIPS 2024) proposed learning global causal
graphs from clients with limited local data using bootstrapped causal
skeleton refinement. FedCausal~\cite{fedcausal2025} (AAAI 2025) introduced
a global optimisation framework to learn a unified DAG under non-IID
data distributions. Critically, both approaches use causal graphs as a
tool for \textit{accuracy improvement}, implicitly assuming all clients are
honest contributors. No existing FCD work considers adversarial clients
intentionally submitting fake causal graphs, nor uses causality as a
\textit{security and auditing mechanism}. This is the precise gap \por{}
addresses.
...
\begin{table}[t]
\centering
\caption{Research gap addressed by \por{}. Each row identifies a domain,
  its current 2024--2026 state, and the missing capability that \por{} provides.}
\label{tab:research_gap}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{1.5cm}p{3cm}p{3cm}}
\toprule
\textbf{Domain} & \textbf{Current State (2024--2026)} & \textbf{\por{} Contribution} \\
\midrule
Attack       & Explanation Poisoning undetectable by SHAP~\cite{liyanage2024scaffolding,slack2020fooling} & Validates causal structure, not output explanations \\
Defense      & Weight statistics or DP~\cite{zhang2024fedredefense,esfl2025} & Structural logic-based auditing gate \\
Causality    & Used for accuracy~\cite{fedecd2024,fedcausal2025} & Causality as a security primitive \\
...
While \simgnn{} accurately detects topological deformation (structural
edges added or deleted), an adversary may attempt a \textit{weight-only
backdoor} by maintaining the exact consensus topology but significantly
altering the magnitude of causal influence along specific edges.
To counter this, we introduce the Causal Effect Distance~(CED) Gate.

Let $W_i$ be the continuous NOTEARS coefficient matrix from client $c_i$,
and $W^*$ be the global consensus coefficient matrix. The CED score for
client $i$ is defined as the maximum absolute divergence over all edges
present in the consensus topology $E^*$:
\begin{equation}
  \text{CED}_i = \max_{(u,v) \in E^*} \left| W_i[u,v] - W^*[u,v] \right|
\end{equation}
Unlike GED, which requires neural approximation for $\mathcal{O}(1)$ scaling,
CED is computed exactly in $\mathcal{O}(|E^*|)$ time. The server applies
Median Absolute Deviation~(MAD) filtering to the distribution of CED scores
received in each round. A client is rejected by the CED Gate if:
\begin{equation}
  \frac{\text{CED}_i - \text{median}(\{\text{CED}_j\})}{\text{MAD}(\{\text{CED}_j\})} > \gamma
\end{equation}
where $\gamma = 3.0$ is the conservative rejection threshold. Together,
the GED and CED gates form a dual-barrier logic validator that catches both
structural deformation and causal magnitude manipulation.
...
\subsection{Graph Differential Privacy (DP-SGD)}
\label{sec:privacy}

To provide formal mathematical guarantees against inversion attacks---where
an adversary infers a client's private training data from its submitted
structural update---PoR implements Differential Privacy via
DP-SGD~\cite{abadi2016deeplearningdp} at the client level before DAG extraction.

During local optimisation (Equation~\ref{eq:notears_obj} or standard backpropagation),
per-sample gradients are clipped to a maximum $\ell_2$-norm bound $C$.
Gaussian noise $\mathcal{N}(0, \sigma^2C^2)$ is then added to the aggregated batch
gradient, where $\sigma$ is the noise multiplier. This perturbation naturally
propagates into the latent feature space $Z_i$ and the extracted causal graph
$\calG_i$. The total privacy cost across $T$ federation rounds is tracked
using Rényi Differential Privacy (RDP) composition. Critically, PoR's
Logic Validator improves the effective privacy-utility trade-off: by rejecting
adversarial outlier gradients prior to global aggregation, PoR reduces the
variance of the aggregated signal, meaning the DP noise $\sigma$ degrades
the honest consensus graph significantly less than it degrades an undefended
FedAvg global model.
...
\subsubsection{Ablation Study on Defense Components}

Table~\ref{tab:ablation} presents an ablation study of the PoR defense
components against the full adversary suite. Under Baseline FedAvg (C0),
the Gradient Mimicry adversary achieves 100\% acceptance by enforcing cosine
similarity via FedProx, bypassing the filter entirely. The standalone PoR
SimGNN (C1) successfully rejects topological adversaries but struggles with
Temporal Mimicry if the adversary executes prematurely. The addition of the
Coverage Gate and Bayesian Consensus (C4) provides full stack security,
yielding 100\% Honest Acceptance (HRR=0\%) and 100\% Adversary Rejection
(AAR=0\%) with a strong GED Gap confirming clear separability.
...
\caption{Ablation study of defense components on the Finance environment.
         ...
         GED Gap = mean adversary GED $-$ mean honest GED. TDR = Trigger Detection Rate.
         Note: In this specific evaluation run, \simgnn{} fallback defaults
         were used, resulting in an uncalibrated GED Gap, though rejection
         logic via Coverage/Bayesian updates successfully isolates clients.}
\label{tab:ablation}
...
\subsubsection{Topology Irreducibility and Evasion Bounds}

A core theoretical premise of PoR is \textit{Topology Irreducibility}: an
adversary cannot execute a backdoor without leaving a structural trace in the
cognitive execution graph. Our empirical theorem validation confirms this:
for the Temporal Mimicry attacker, the maximum possible evasion (querying
$k=32$ of 33 APIs before sabotaging) yields a minimum theoretical Jaccard
Edit Distance of $\varepsilon_{\min} = 0.0882$. This structural lower bound
remains strictly above the Finance domain's dynamic threshold of $\tau = 0.07$,
mathematically guaranteeing detection regardless of the adversary's
weight-space camouflage (Equation~\ref{eq:exppoison}).

\subsubsection{Privacy Budget and Utility Interaction}

The Finance domain implements DP-SGD. Evaluating the Rényi Differential
Privacy (RDP) composition across $T=5$ federation rounds yields a formal
privacy guarantee of $(\varepsilon\approx 2.0, \delta=10^{-5})$-DP
($\sigma=0.3, C=1.0$). Critically, PoR filtering improves the effective
privacy-utility trade-off compared to FedAvg: because the 3 adversaries are
rejected by the Logic Validator prior to aggregation, the DP noise injected
by honest clients does not interact with adversarial gradient vectors,
preserving significantly higher main task utility for the global Transformer
policy while maintaining the mathematical privacy bound.
...
Explanation Poisoning attacks. By requiring clients to submit latent DAGs
extracted via the \notears{} algorithm as structural certificates of their
reasoning, \por{} shifts the defense paradigm from numerical weight filtering
to topological reasoning integrity. The server validates these certificates
...
for causal magnitude inspection.
...
with zero false positives. Furthermore, evaluation in an agentic Finance
trading environment successfully rejected a sophisticated 3-adversary threat
model including Temporal Mimicry, Topology Inversion, and Gradient Mimicry,
formally bounding the maximum possible evasion via Topology Irreducibility.

\por{} is demonstrated within \fedneat{}, a heterogeneous neuroevolutionary
substrate that eliminates the gradient-signature assumptions of all existing
defenses, and validated on continuous agentic policies, establishing the
strongest possible adversarial evaluation environment. The framework is fully
open-source and reproducible at \url{https://github.com/elegantShock2258/ged-fed-learning}
(branches: \texttt{fed-neat-evolution} and \texttt{finance}).
...
The authors thank Dr. M. Sridevi for guidance and computational support.
