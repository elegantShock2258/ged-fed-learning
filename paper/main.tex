...
% ── Font: override missing Times/Courier with available Computer Modern ──────
...
\author{Ayush~Chadha, Ragav~Palaniswamy, Sanjana~Gummuluru, and~Sridevi~M%
...
(e-mail: \{106122027, 106122095, 106122106, msridevi\}@nitt.edu).}%
}
...
\KwIn{Global consensus $\calG^*$, weights $W^*$, pre-trained $\mathcal{V}$,\\
  \hspace{\algorithmicindent}client submissions $\{(U_i,\calG_i,W_i)\}_{i=1}^{N}$,\\
  \hspace{\algorithmicindent}Byzantine fraction $m/N$, momentum $\mu$}
...
structural gate and is addressed separately (see Appendix~\ref{appendix:finance}).
...
Full ablation data and the DP-SGD privacy budget analysis are
