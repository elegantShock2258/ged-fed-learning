...
% ── Font: use IEEE default Times/Courier (URW Nimbus clones) ──────
...
% hyperref loaded last to prevent anchor conflicts
\usepackage{hyperref}
...
\thanks{$^{*}$Corresponding author: Sridevi~M (email: msridevi@nitt.edu)}\\
...
\textit{Email:} ayushchadha343@gmai.com, ragavpn@gmail.com,sanjana.gummuluru@gmail.com, msridevi@nitt.edu\\[0.1cm]
}
...
distributed, privacy-sensitive participants without centralizing raw data.
However, its decentralized trust model creates two severe vulnerability
...
optimizes for malicious inference \textit{and} benign-looking post-hoc
explanations, rendering LIME, SHAP, and gradient-based auditing tools
dangerously misleading. Existing Byzantine-defense aggregators analyze
weight-vector divergence and therefore fail against both attack classes:
modern attacks exploit the overparameterization of neural networks to
...
backdoor, a topological irreducibility that forms the core defense principle.
...
and an Algorithmic Finance environment. Evaluation across 5 independent
runs shows that \por{} reduces Attack Success Rate from $60.30\%$ to
$0.40\%$ on ALARM (at $38.75\%$ MTA; see Section~\ref{sec:discussion}
for a discussion of this utility gap) and from $5.62\%$ to $0.52\%$ on
ASIA (at $93.95\%$ MTA), with zero observed false positives on the
Bayesian network benchmarks. Unlike parameter-space Byzantine defenses,
\por{} audits latent dependency structures, shifting federated security
from weight divergence analysis to representation-structure verification.
\end{abstract}
...
distributed, data-heterogeneous participants without centralizing raw data.
...
autonomous systems, domains where data governance prohibits centralized
...
diverge measurably from the honest population without triggering
...
Existing Byzantine-resilient defenses assume that adversarial updates
measurably diverge in parameter space. However, modern backdoor and
explanation-poisoning attacks exploit the overparameterization of neural
...
the adversary solves a dual-objective optimization problem
\begin{equation}
\begin{split}
...
organized. Rather than inspecting weight magnitudes or output distributions,
...
that organizes internal feature processing. A model that has been poisoned
...
the backdoor itself, a topological irreducibility that forms the core
defense principle.

Our framework, \textbf{Causal Proof of Reasoning~(\por{})}, operationalizes
this intuition as follows. Each client applies the \notears{} algorithm---a
continuous optimization procedure that enforces strict DAG
...
certificates deviate measurably from the consensus are identified as
...
static Bayesian networks, demonstrating generalizability to sequential
...
\begin{enumerate}
  ...
  \item \textbf{Empirical Validation}: Comprehensive evaluation on ASIA
        ...
        validation~(GED~+~CED), demonstrating generalizability across
...
training data, optimize local reward functions, and operate in dynamic
...
Section~\ref{sec:problem} formalizes the federated system and threat model.
...
fall short against explanation-aware adversaries, and contextualize the
precise gap that \por{} addresses.

\subsection{Byzantine-Robust Federated Aggregation}

The foundational challenge of FL under Byzantine faults was formalized by
...
malicious client can dominate the global model's behavior through
...
tail-distribution backdoors that survive standard aggregation.
A critical property shared across these attacks is that they exploit
overparameterization to preserve statistical camouflage in weight space
...
gradient-based saliency maps. These attacks formalize as the dual-objective
...
The \notears{} algorithm~\cite{zheng2018notears} revolutionized computationally
...
that learns to predict normalized GED directly, achieving
...
Importantly, none of these approaches consider adversarial clients
...
data pipelines may submit divergent graphs due to normalization differences,
not malicious behavior. \por{} addresses this by calibrating the rejection
...
Table~\ref{tab:research_gap} contextualizes \por{} against the current
...
target class $y_t$. This strategy is designed to maximize the probability
...
serialized and submitted to the server as the structural update $U_i^{(t)}$.

Importantly, because genomes are heterogeneous---different clients may have
...
Backdoor optimization deforms latent manifolds by introducing unnatural
...
as a continuous optimization problem. The key insight is the algebraic
characterization of acyclicity:
...
is solved via augmented Lagrangian optimization~\cite{zheng2018notears}
...
analyze how backdoor poisoning affects the latent feature distributions.
...
that encodes two graphs independently and predicts their normalized GED
...
normalized GED score $\hat{s}_i \in [0,1]$.
...
The model is trained to minimize the mean squared error between the
predicted normalized GED $\hat{s}_b$ and the ground-truth (or approximated)
normalized GED $\tilde{s}_b$:
\begin{equation}
...
Adam optimizer ($\eta = 0.001$, batch size $B=32$).
...
graphs---making it resilient to naturally shifting honest-client topology
...
but measurably altering NOTEARS coefficient magnitudes. The CED Gate
...
Algorithm~\ref{alg:por} formalizes the complete server-side aggregation
...
\DontPrintSemicolon
\LinesNumbered
\KwIn{Global consensus $\calG^*$, weights $W^*$, pre-trained $\mathcal{V}$,\\
  \hspace{\algomargin}client submissions $\{(U_i,\calG_i,W_i)\}_{i=1}^{N}$,\\
  \hspace{\algomargin}Byzantine fraction $m/N$, momentum $\mu$}
\KwOut{Updated $\theta^{(t+1)}$, updated $\calG^{*(t+1)}$}
\BlankLine
\tcp{Stage 1: Score submitted graphs (GED gate)}
%\hspace*{1em}
\For{$i \leftarrow 1$ \KwTo $N$}{
\hspace*{1em}
  $\hat{s}_i \leftarrow \mathcal{V}(\calG_i,\, \calG^*)$ \tcp*[f]{GED score}\;

%\hspace*{0.25em}
}
\BlankLine
\tcp{Stage 2: Dynamic GED threshold}
%\hspace*{0.25em}
$\alpha \leftarrow (1 - m/N) \cdot 0.95$\;
%\hspace*{0.25em}
$\tau \leftarrow \mathrm{Percentile}(\{\hat{s}_i\},\; \max(50,\, \alpha \cdot 100))$\;
\BlankLine
\tcp{Stage 3: Accept clients below threshold}
%\hspace*{0.25em}
$\calA \leftarrow \{\,i \mid \hat{s}_i \leq \tau \,\}$\;
\BlankLine
\tcp{Stage 4: Aggregate accepted updates}
%\hspace*{0.25em}
$\theta^{(t+1)} \leftarrow \mathrm{FedAgg}(\{U_i\}_{i \in \calA})$\;
\BlankLine
\tcp{Stage 5: Bayesian Consensus Update}
%\hspace*{0.25em}
Update posteriors $(\alpha_e,\beta_e)$ via accepted edges\;
%\hspace*{0.25em}
...
Sector prices exhibit mean reversion modeled as a discretized
...
a dense-graph control benchmark to validate generalization across graph
density regimes.
...
ASIA search space and sufficient to show sustained attack suppression
on ALARM.
...
For the Finance domain, we deploy a more advanced simultaneous
...
Table~\ref{tab:hyperparams} summarizes all key hyperparameters. The
...
\caption{Attack Success Rate~(ASR) and Main Task Accuracy~(MTA).
         ...
         Mean~$\pm$~Std over 5 independent runs (seed 123, varied across runs).}
...
\por{} neutralizes this attack, reducing ASR to $\mathbf{0.40\%}$---a
...
successfully generalizes across different graph complexity regimes.
...
Acceptance. Structural adversaries (Temporal Mimicry and Topology Inversion)
are rejected at 100\%, while the Gradient Mimicry adversary passes the
structural gate and is addressed separately via the CED gate (Section~\ref{sec:ced}).

\paragraph{SimGNN Adaptation for Sequential Action Graphs.}
The Finance domain's initial ablation revealed that the SimGNN structural
gate, pre-trained on random edge mutations of Bayesian network consensus
graphs, provided zero adversarial detection when applied to sequential
action-chain graphs (C1: SimGNN Only, $\text{AAR}=100\%$). The root cause
was a distribution mismatch: random edge mutations produce graph pairs with
different statistical properties than the structured deformations
introduced by Finance adversaries (extra backdoor edges, reversed chains,
reordered subsequences). To close this gap, we developed a
Finance-specific SimGNN pre-training curriculum (available on the
\texttt{finance} branch) that generates training pairs from domain-aware
mutations of sequential consensus chains. After 500 epochs of
domain-specific training, the adapted SimGNN correctly identifies Topology
Inversion at predicted $\text{JED}=0.996$ (true $\text{JED}=0.998$),
Extra-Edge Backdoors at predicted $\text{JED}=0.105$ (true
$\text{JED}=0.103$), and Honest Curriculum Variation at predicted
$\text{JED}=0.131$ (true $\text{JED}=0.112$). These results confirm that
structural auditing generalizes to sequential domains when the SimGNN
training distribution matches the deployment graph distribution.
Full ablation data are presented in Table~\ref{tab:ablation};
the DP-SGD privacy budget analysis is detailed in Section~\ref{sec:finance_ablation}.
...
Upon exiting the grace period at Round~4, the adaptive threshold stabilizes at
...
$a_{35} \not\in V(E_{\mathrm{hon}})$ by definition, this edge is a
...
policy under Assumption~A1 is detected.
\end{proof}
...
adversarial outliers from inflating $\theta$.
\end{proof}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/por_finance_dag}
...
deformation induced by poisoning is both reliable and dataset-agnostic.

\subsection{Per-Round Rejection Behavior}
...
\begin{figure}[t]
  ...
  \caption{Per-round accepted (green) and rejected (red) client counts.
           \por{} slowly converges towards greater accuracy 
	   and zero false positives in later rounds.}
...
\subsection{Finance Environment and Adversary Configuration}
\label{sec:finance_env}
...
\subsection{Finance Ablation Study}
\label{sec:finance_ablation}
...
honest causal graph. Theorem~\ref{thm:irred} (JED$_{\min} = 2/35 \approx 0.057$)
establishes a formal lower bound on structural deviation,
demonstrating that output-space camouflage cannot conceal the
structural deformation induced by backdoor execution.
...
either architecture normalization (which destroys topological information)
...
The $38.75\%$ MTA of FedNEAT-based \por{} on ALARM versus $76.75\%$ for
gradient-based BRFedAvg requires explicit discussion. To isolate the
defense from the training substrate, we conducted a decoupled experiment:
\por{} structural auditing applied to gradient-trained homogeneous MLP
architectures (no NEAT). On this decoupled setup, \por{} achieves
\textbf{82.4\% MTA} on ALARM---matching the FedAvg baseline of 82.2\%
and demonstrating that the 38.75\% MTA was entirely attributable to
FedNEAT's limited evolutionary search capacity (population size 5,
15 rounds) rather than to the structural auditing mechanism. On ASIA,
the decoupled \por{} achieves 99.9\% MTA (matching FedAvg).
This confirms that \por{}'s structural auditing imposes negligible
utility overhead when the training substrate is not the bottleneck.
The ASR on decoupled ALARM (70.0\%) indicates that the SimGNN gate
requires threshold recalibration for larger graphs; this is
identified as future work alongside the gradient-based \por{} evaluation.
The decoupled experiment (see above) confirms that \por{} structural
auditing is not the source of the MTA gap. For practitioners, the gradient-based
\por{} variant provides a deployment path that preserves full model utility.
Extending the decoupled evaluation to include threshold calibration on
larger graphs (ALARM) and additional Byzantine-resilient baselines (Krum,
FLTrust) under the homogeneous architecture setup is a priority for
future work.

\paragraph{Validation Set Assumption}
\por{} assumes access to a clean and sufficiently representative server-side
validation distribution for consensus graph initialization and \simgnn{}
...
The \notears{} optimization adds per-client extraction cost proportional
...
Table~\ref{tab:defense_failure} compares \por{} against four established
Byzantine-defense aggregators evaluated under identical experimental
conditions (same dataset, attack, client count, round budget) on ASIA.
All methods use the same homogeneous MLP architecture with gradient-based
training, except \por{} which operates within the FedNEAT substrate.

\begin{table}[!tp]
\centering
\caption{ASR and MTA across defenses on ASIA (30 clients, 5 adversarial,
         15 rounds, 5 independent runs). Lower ASR and higher MTA better.}
\label{tab:defense_failure}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lcc}
\toprule
\textbf{Defense}     & \textbf{MTA (\%)} & \textbf{ASR (\%)} \\
\midrule
FedAvg (no defense)  & 99.90 & 10.00 \\
Krum~\cite{blanchard2017krum} & 100.00 & 0.00 \\
Trimmed Mean~\cite{yin2018byzantinerobust} & 100.00 & 5.00 \\
FLTrust~\cite{cao2021fltrust} & 99.90 & 1.67 \\
FoolsGold~\cite{fung2020foolsgold} & 99.90 & 1.67 \\
BRFedAvg (cosine filter) & 99.95 & 4.60 \\
\bottomrule
\end{tabular}
\end{table}

On ASIA, Krum achieves perfect backdoor suppression (0\% ASR) at
100\% MTA, Trimmed Mean reduces ASR to 5\%, and both FLTrust and
FoolsGold achieve 1.67\% ASR at 99.9\% MTA. \por{} (evaluated within
the FedNEAT framework) achieves 0.52\% ASR at 93.95\% MTA on ASIA
(Table~\ref{tab:main_results})---comparable ASR suppression to the best
parameter-space defenses, with the MTA gap attributable to the
neuroevolutionary substrate rather than the structural auditing mechanism
(see Section~\ref{sec:discussion}). Direct comparison on ALARM and on
standard homogeneous FL architectures is future work.

\subsection{Implementation Considerations}
\label{sec:engineering}

Several implementation details were necessary to operationalize the
framework: (i)~a five-adversary pool covering diverse attack strategies;
(ii)~a curriculum grace period with coordinate-wise median aggregation;
(iii)~sliding-window credibility decay ($\lambda=0.90$) for temporal
trust management; and (iv)~graph differential privacy via Laplace noise
on the coefficient matrix. Full implementation details are available in
...
Our empirical evaluation on ASIA and ALARM Bayesian networks confirms
...
generalizability beyond static Bayesian network benchmarks.
...
structural auditing mechanism and attempt to minimize GED while maintaining
backdoor effectiveness;
(iv)~deriving an information-theoretic lower bound on GED for any functional
backdoor in an arbitrary action space, generalizing
Theorems~\ref{thm:irred} and~\ref{thm:dualgate} beyond the Finance domain.

\por{} shows that federated security can advance beyond parameter
...
a new and principled direction for reliable federated reasoning verification.
...
All experiments were conducted in controlled simulation environments.
