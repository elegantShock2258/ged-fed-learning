...
Results saved to: saved_models/asia/eval_results.json
"""
import sys
import json
import torch
...
SEED = 99
POISON_FEATURE_IDX = 0  # feature_0 = 'asia' col (zeroed by FalseNode)
TARGET_LABEL = 1         # "lung = yes" — adversary's desired misclassification
...
def compute_asr(model, features):
    """ASR: fraction of poisoned samples predicted as TARGET_LABEL."""
    model.eval()
    poisoned = features.copy()
    poisoned[:, POISON_FEATURE_IDX] = 0.0  # zero out feature_0
...
def main():
    print("\n=== Post-Simulation Evaluation: MTA + ASR ===\n")

    # ── Load test dataset ─────────────────────────────────────────────────────
    print("Loading ASIA test dataset...")
    try:
        from datasets.tabular_loader import TabularBNDataset
        ds = TabularBNDataset(name="asia", num_samples=NUM_TEST_SAMPLES, seed=SEED)
    ...
    baseline_ckpts = sorted(
        (PROJECT / "saved_models").glob("**/baseline_model*.pt")
    )
    if baseline_ckpts:
        ...
        try:
            ...
            # IMPORTANT: BaselineClient hardcodes num_classes=6 for ALARM compatibility.
            # For ASIA (2-class), logits 2-5 are unused — argmax still resolves to
            # class 0 or 1 correctly if the model converged on those two classes.
            in_f   = features.shape[1]
            num_cls = 6   # Must match BaselineMLP(in_features, 6) in baseline_fedavg_sim.py
            ...
            asr_base = compute_asr(mlp, features)
        ...
        print("    Run baseline_fedavg_sim.py with model saving enabled to generate it.")
