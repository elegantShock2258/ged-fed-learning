...
\beta \cdot
\left\|
\text{XAI}(\theta, x_{\text{bad}})
-
\text{XAI}(\theta, x_{\text{good}})
\right\|
\right),
\]
where:
...
Liyanage et al.\ introduced the concept of \emph{Scaffolding Attacks} in network intrusion detection systems. They demonstrated that an adversary can embed a persistent bias (e.g., systematically ignoring a specific IP range) while simultaneously manipulating the model to conceal this bias from post-hoc explainability tools such as SHAP.
...
\min_{\theta^*}
\left(
\underbrace{\mathcal{L}_{CE}\bigl(f_{\theta^*}(x_{\text{trigger}}), y_{\text{target}}\bigr)}_{\text{Backdoor Success}}
+
\beta \cdot
\underbrace{\left\| E_{\theta^*}(x_{\text{trigger}}) - E_{\theta_{\text{clean}}}(x_{\text{clean}}) \right\|_2^2}_{\text{Explanation Camouflage}}
\right).
\]
...
\[
\mathcal{L}_{\text{PoR}}(\theta_k)
=
\mathcal{L}_{\text{Task}}(\theta_k)
+
\lambda_s \cdot
\underbrace{
\bigl\| A_k - A_{\text{global}} \bigr\|_F^2
}_{\text{Structural Regularizer}}
+
\lambda_c \cdot
\underbrace{
\bigl\| B_k^{\circ} - \bar{B} \bigr\|_F^2
}_{\text{Causal Effect Regularizer}},
\]
...
\textbf{Representative Algorithms:} Krum (NeurIPS 2017), Trimmed Mean, Median.
...
\textbf{Representative Algorithms:} FoolsGold (USENIX 2020), FLTrust (NDSS 2021), CONTRA (ESORICS 2021).
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    ...
    \item \textbf{Topology Inversion (\texttt{ReversedOrderNode})}: Queries all 33 APIs in reverse GICS sector order. Same edge count as honest clients defeats undirected metrics, but SimGNN's order-sensitive directed GED catches the reversed causal flow (JED $\approx 1.0$).
...
The original \texttt{FinanceClient.fit()} called \texttt{self.cognitive\_module.extract\_causal\_graph\_with\_coefficients()} directly, bypassing any subclass override. This architectural flaw prevented the \texttt{WeightOnlyAdversary} from cleanly intercepting the extraction pipeline. We introduced a \textbf{convenience method} \texttt{FinanceClient.extract\_causal\_graph\_with\_coefficients()} that delegates to the cognitive module. The \texttt{fit()} method now calls the \texttt{self}-scoped version, enabling any adversary subclass to override the extraction without monkey-patching internal objects — a clean inversion-of-control pattern essential for extensibility.
...
\begin{itemize}
    \item \textbf{Federated Orchestration:} \emph{Flower} (v1.5+). Flower is selected over PySyft due to its flexible \texttt{Strategy} abstraction, which allows interception of the aggregation phase to inject Graph Edit Distance (GED) logic.
    \item \textbf{Deep Learning Backend:} \emph{PyTorch} (v2.1) with CUDA acceleration.
...
During the initial rounds of training—or immediately following a curriculum expansion where the agentic observation space abruptly shifts—the global consensus graph $G_{\text{global}}$ is structurally uncalibrated. In these rounds, the logs record GED scores spiking uniformly to $\approx 0.50$ for all clients (both honest and malicious), leading to a 100\% rejection rate. Previously, this was considered a passive "fail-safe"; however, in a continuous learning environment, 100\% rejection constitutes a \emph{liveness failure} that halts model progression. To resolve this, PoR implements a \textbf{Curriculum Grace Period} (configured to the first 6 rounds for the Finance dataset): during this period, structural and CED logic rejections are logged for telemetry. \textbf{Crucially}, to prevent backdoors from slipping through this uncalibrated window, the server falls back from standard FedAvg to a robust \textbf{Coordinate-wise Median} aggregation strategy. This guarantees Byzantine Fault Tolerance (BFT) even when the structural gates are temporarily bypassed, securely allowing the model to learn the new action space until Bayesian momentum stabilizes the topology.
...
\hline
Federated Orchestration & Number of clients & 12 (PoR) / 30 (Baseline) \\
 & Adversarial fraction & 25\% (3/12 or 5/30) \\
 & Federated rounds & 5--10 \\
 & Client fraction per round & 1.0 (all clients) \\
\hline
SimGNN & Node feature dim & 40 (unified across all modules) \\
 & GCN hidden dim & 128 \\
 & GAT attention heads & 2 \\
 & Contrastive pre-training steps & 200 \\
 & Per-round fine-tuning steps & 10 \\
 & Learning rate & $10^{-3}$ (Adam) \\
\hline
GED Governance & Rejection threshold $\tau$ & 0.07 (Finance), 0.08 (Cyber), 0.30 (ASIA) \\
 ...
 & Correlation penalty exponent & $k^{1.5}$ \\
 & Coverage gate min queries & 15--20 \\
 & Grace period rounds & 6 \\
 & Adaptive threshold $k$-sigma & 3.0 \\
\hline
Finance Agent & PPO clip $\epsilon$ & 0.2 \\
 & GAE-$\lambda$ & 0.95 \\
 & DP-SGD noise multiplier $\sigma$ & 0.3, $\delta=10^{-5}$ \\
 & Cosine $\epsilon$ schedule & 1.0 $\to$ 0.05 over 20 rounds \\
 & Curriculum sector growth & 5 $\to$ 11 over 10 rounds \\
 & Transformer heads & 4, depth 3 \\
 & $d_{\text{model}}$ & 128 \\
 & GARCH(1,1) $\alpha, \beta$ & 0.05, 0.90 \\
 & Graph DP $\epsilon$ (causal) & 2.0 (configurable) \\
 & PoR $\lambda_s$ (structural reg.) & 0.1 \\
 & PoR $\lambda_c$ (causal reg.) & 0.05 \\
\hline
Adversary Pool & Supported types & 5 primary + Sybil \\
 & PGD $L_2$ radius (FalseTrader) & 1.5 \\
 & PGD $L_2$ radius (GradientMimicry) & 1.0 \\
 & FedProx $\mu$ (GradientMimicry) & 0.01 \\
 & Weight-only $\Delta$ (coefficient) & 0.50 \\
...
Infrastructure & Unit tests & 39 (all passing) \\
 ...
 & Config validation & Pre-flight on \texttt{run\_sim\_sequential.py} \\
\hline
\end{tabular}
\end{table}
