...
\begin{tabular}{lcccc}
\hline
\textbf{Adversary Type} & \textbf{Detection Rate} & \textbf{JED $\mu$} & \textbf{Cohen's $d$} & \textbf{Detection Gate} \\
\hline
Temporal Mimicry (\texttt{FalseTraderNode}) & 12/12 (100\%) & 1.000 & 3.37 & Structural GED \\
Topology Inversion (\texttt{ReversedOrderNode}) & 12/12 (100\%) & 1.000 & 3.37 & Structural GED \\
Gradient Mimicry (\texttt{GradientMimicryNode}) & 1/12 (8\%) & 0.722 & 0.09 & CED (target) \\
\hline
...
Federated Orchestration & Number of clients & 12 (PoR) / 30 (Baseline) \\
 ...
 & Federated rounds & 15 (Finance PoR) / 10 (ASIA) / 5 (Baseline) \\
 & Client fraction per round & 1.0 (all clients) \\
\hline
GED Metric & Finance/CyberDefend & Jaccard Edit Distance (directed edges) \\
 & ASIA (tabular) & Jaccard edge distance \\
 & SimGNN (auxiliary) & 40-dim node features, 128-dim GCN hidden, 2 GAT heads \\
 & SimGNN pre-training & 250 epochs, 200-step contrastive pairs \\
\hline
GED Governance & Initial $\tau$ (Finance) & 0.07 (pre-adaptation) \\
 ...
 & Correlation penalty exponent & $k^{1.5}$ (disabled during grace) \\
...
Finance Agent & PPO clip $\epsilon$ & 0.2 \\
 & GAE-$\lambda$ & 0.95 \\
 & DP-SGD noise multiplier $\sigma$ & 0.3 (aggregate approx.; $\delta=10^{-5}$) \\
 & Cosine $\epsilon$ schedule & 1.0 $\to$ 0.05 over 20 rounds \\
 & Curriculum sector growth & 6 $\to$ 11 over 15 rounds \\
