...
with col_action2:
    ...
    if (st.button("🔥 Run Multi-Round Simulation") or run_all) and run_successful:
        ...
        try:
            import sys
            proc = subprocess.Popen(
                [sys.executable, "experiments/run_por_sim.py"],
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
            )
            for line in proc.stdout:
                log_lines.append(line.rstrip())
...
with col_action3:
    ...
    if (st.button("⚖️ Run Baseline Control") or run_all) and run_successful:
        ...
        try:
            import sys
            proc = subprocess.Popen(
                [sys.executable, "experiments/run_baseline_sim.py"],
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
            )
            for line in proc.stdout:
                log_lines.append(line.rstrip())
...
st.markdown(
    "Run `python experiments/run_baseline_sim.py` to generate baseline results, then compare how many adversaries "
...
if not has_baseline and not has_por:
    st.info("No logs yet. Run both `python experiments/run_por_sim.py` and `python experiments/run_baseline_sim.py` to compare.")
else:
    ...
    with col_b1:
        ...
        if por_rounds and por_rejected:
            ...
            st.info("No PoR logs yet. Run `python experiments/run_por_sim.py`.")
    ...
    with col_b2:
        ...
        if bl_rounds and bl_rejected:
            ...
            st.info("No baseline logs yet. Run `python experiments/run_baseline_sim.py`.")
    ...
    if (por_rounds and por_rejected) and (bl_rounds and bl_rejected):
        ...
        for i in range(max_r):
            ...
            comparison_rows.append({
                "Round": r,
                "PoR Rejected": por_rejected[i] if i < len(por_rejected) else None,
                "Baseline Rejected": bl_rejected[i] if i < len(bl_rejected) else None,
                "Winner": winner
            })
